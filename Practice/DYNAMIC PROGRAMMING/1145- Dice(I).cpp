...
int main()
{
    ...
    while(T--)
    {
        ...
        while(i<=N and (S-K*i-1)>0 and (S-K*i)>=N)
        {n
            lli x=(FACT[N]*Power((FACT[i]*FACT[N-i])%mod,mod-2))%mod;
            lli y=(FACT[S-K*i-1]*Power((FACT[N-1]*FACT[S-K*i-N])%mod,mod-2))%mod;

            if(i%2)
            {
                Sum=(Sum-(x*y)%mod+mod)%mod;
            }
            else
            {
                Sum=(Sum+(x*y)%mod)%mod;
